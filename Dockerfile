...
RUN go mod download
COPY . .
RUN CGO_ENABLED=0 go build -ldflags='-s -w' -o /out/nfe ./cmd/nfe
...
# already contains linux/amd64/nfe (the binary goreleaser cross-built).
FROM gcr.io/distroless/static-debian12 AS goreleaser-runtime
COPY linux/amd64/nfe /usr/local/bin/nfe
ENTRYPOINT ["/usr/local/bin/nfe"]
